...
## Read in the SMRs
  smr.data <- read.table("smr.txt",header=TRUE,sep=" ")
...
## Create the SMR plot backbone
  plot(seq(1,23,1),smr.data$smr,pch=22,col="black",bg="black",xaxt="n",yaxt="n",
       ylim=c(0,4),xlab="",ylab="",cex=1.5)
  points(23,smr.data$smr[23],pch=22,col="red",bg="red",cex=1.5)

## Add axes and tick-mark labels
  axis(1,at=1:23,labels=FALSE)
  axis(2,at=seq(0,4,0.5), labels=FALSE)
  text(1:22,par("usr")[3]-0.25,srt=45,adj=1,labels=smr.data$period[1:22],xpd=TRUE,cex=0.9)
  text(23,par("usr")[3]-0.25,srt=45,adj=1,labels=smr.data$period[23],xpd=TRUE,cex=0.9,col="red")
  axis(2,las=1,at=c(0,0.5,1,1.5,2,2.5,3,3.5,4),labels=c("0.0","0.5","1.0","1.5","2.0","2.5","3.0","3.5","4.0"))
...
## Put in all the confidence bands
  ...
  segments(23,smr.data$lci[23],23,smr.data$uci[23],col="red") 